...
// PostUserCreated runs the method Post of the given controller with the given parameters and payload.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func PostUserCreated(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.UserController, payload *app.PostUserPayload) (http.ResponseWriter, *app.UserMedia) {
	t.Helper()
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "UserTest"), rw, req, prms)
	postCtx, _err := app.NewPostUserContext(goaCtx, req, service)
	if _err != nil {
